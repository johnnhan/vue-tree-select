...
| alignMode | String | 'left' | 组件下拉列表对齐方式，'left'（左对齐），'center'（居中），'right'（右对齐）|
...
| preSelectFun | Function | 空函数 | 选择前置函数，支持 Promise 函数，通过该函数可实现选择某一项前的弹窗提示等功能 |
| placeholder | String | '请选择' | 未选择时的替代文本 |
| clearIcon | Object | [附录二](#附录二（自定义图标数据传递格式）) | 自定义清空图标 |
| arrowUpIcon | Object | [附录二](#附录二（自定义图标数据传递格式）) | 自定义下拉箭头图标 |
| arrowRightIcon | Object | [附录二](#附录二（自定义图标数据传递格式）) | 自定义树折叠图标 |
